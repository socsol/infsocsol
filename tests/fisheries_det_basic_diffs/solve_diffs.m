...
function diffs = solve_diffs(varargin)
  
  DeltaFunction = 'fisheries_delta';
  StageReturnFunction = 'fisheries_cost';
  
  lb = [60 0.1];
  ub = [600  1];

  opts = struct(varargin{:});
  state_step = (ub - lb) ./ (opts.States - 1);

  StateLB = lb - state_step;
  StateUB = ub + state_step;
  ...
  try
    ...
    diffs = cell(1, Options.PolicyIterations);
